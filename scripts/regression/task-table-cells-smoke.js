...
assert.match(cellsSource, /export function TaskTargetDateCell/, 'Task table cells must keep target date cell ready for extraction.')
assert.match(cellsSource, /export function TaskStatusCell/, 'Task table cells must keep status cell ready for extraction.')
...
assert.match(pageSource, /TaskResponsibleCell } from '@\/features\/tasks\/task-table-cells'/, 'Task page must import responsible cell from shared cells.')
