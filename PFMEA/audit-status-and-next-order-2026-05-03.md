...
| PFMEA helper extraction | PARTIAL / ADVANCED | Duza czesc czystej logiki jest w `src/features/pfmea/*-utils.ts`; dodatkowo `pfmea-service` obsluguje author/role/project view/history, edit session startup, draft ensure, pobieranie wierszy rewizji i restore snapshot. Strona nadal laczy UI, stan i czesc orkiestracji save/publish. |
...
| 7 | B-005 | PFMEA service layer | PARTIAL / ADVANCED | Wydzielono bezpieczne elementy serwisu: edit session startup, ensure draft, pobieranie wierszy rewizji i restore snapshot. Pozostaje najtrudniejsza czesc: pelna orkiestracja save/publish i dalsze ograniczenie logiki Supabase w stronie. |
...
4. Kontynuowac PFMEA service layer ostroznie: nastepny bezpieczny fragment to batch update dirty rows / row order persistence, a dopiero potem pelna save/publish orchestration.
