...
| PFMEA helper extraction | DONE / MONITOR | Duza czesc czystej logiki jest w `src/features/pfmea/*-utils.ts`; dodatkowo `pfmea-service` obsluguje author/role/project view/history, edit session startup, draft ensure, pobieranie wierszy rewizji, restore snapshot, batch update dirty rows, row order persistence, publish RPC/history wrapper, fallback history insert, cleanup draft rows i cleanup edit session po publikacji. Fizyczne odchudzanie UI obejmuje: top summary, save revision modal, confirm dialog, revision history modal, toolbar/column filter, table header, delete cell, wspolne popup positioning helpers, merged cell helpers, `TdRead`, `TdText`, `TdScaleSelect`, `TdClassSelect`, `TdPcpToggle`, `TdDate`, `TdSelect`, column config, column visibility hook, sticky merged cell hook, dirty draft persistence hook, pending cell values hook, transient tracking hook, pending cell update queue hook, risk matrix config hook, scale options hook, save timing logger, save draft revision resolver, save auth/editor commit helpers, post-publish orchestration helper, published row metadata sync helper, published integrity helper, draft snapshot remap helper, draft snapshot persist helper, page style constants, PFMEA domain types oraz wspolny `editorBase` wydzielone z `app/pfmea/page.tsx`. Pozostale lokalne funkcje sa glownie koordynacja widoku/tabeli i powinny byc rozbijane tylko przy kolejnych zmianach funkcjonalnych. |
...
4. PFMEA traktowac jako etap domkniety architektonicznie; kolejne rozbicia wykonywac tylko przy zmianach funkcjonalnych lub po dodaniu testow przegladarkowych dla edycji tabeli.
