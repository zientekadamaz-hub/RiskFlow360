...
| PFMEA helper extraction | PARTIAL / ADVANCED | Duza czesc czystej logiki jest w `src/features/pfmea/*-utils.ts`; dodatkowo `pfmea-service` obsluguje author/role/project view/history, edit session startup, draft ensure, pobieranie wierszy rewizji, restore snapshot, batch update dirty rows, row order persistence, publish RPC/history wrapper, fallback history insert i cleanup draft rows po publikacji. Rozpoczeto fizyczne odchudzanie UI: top summary, save revision modal, confirm dialog, revision history modal, toolbar/column filter, table header, delete cell, wspolne popup positioning helpers oraz merged cell helpers sa wydzielone z `app/pfmea/page.tsx`. |
