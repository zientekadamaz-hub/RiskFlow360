...
| PFMEA helper extraction | PARTIAL / ADVANCED | Duza czesc czystej logiki jest w `src/features/pfmea/*-utils.ts`; dodatkowo `pfmea-service` obsluguje author/role/project view/history, edit session startup, draft ensure, pobieranie wierszy rewizji, restore snapshot, batch update dirty rows, row order persistence, publish RPC/history wrapper, fallback history insert, cleanup draft rows i cleanup edit session po publikacji. Rozpoczeto fizyczne odchudzanie UI: top summary, save revision modal, confirm dialog, revision history modal, toolbar/column filter, table header, delete cell, wspolne popup positioning helpers, merged cell helpers, `TdRead`, `TdText`, `TdScaleSelect`, `TdClassSelect`, `TdPcpToggle`, `TdDate`, `TdSelect`, column config, column visibility hook, sticky merged cell hook, dirty draft persistence hook, pending cell values hook, transient tracking hook, pending cell update queue hook, risk matrix config hook, scale options hook, save timing logger, save draft revision resolver, save auth/editor commit helpers, post-publish orchestration helper, published row metadata sync helper, published integrity helper, draft snapshot remap helper, page style constants, PFMEA domain types oraz wspolny `editorBase` sa wydzielone z `app/pfmea/page.tsx`. |
